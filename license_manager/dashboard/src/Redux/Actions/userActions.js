...
export const sendLink = (email) => async (dispatch) => {
  ...
  try {
    dispatch({ type: USER_FORGETPASSWORD_REQUEST });
    ...
    localStorage.setItem("userInfo", JSON.stringify(data));
    toast(
      "🦄 Password Rest Link Send Succesfully in Your Email..!",
      ToastObjects
    );
  } catch (error) {
    dispatch({
...
// RESET PASSWORD
export const resetPassword = (token, id, password) => async (dispatch) => {
  const ToastObjects = {
    position: "top-right",
    autoClose: 4000,
    hideProgressBar: false,
  ...
  try {
    ...
    toast.success("🚀 Password Updated Succesfully", ToastObjects);
  } catch (error) {
    dispatch({
